...
const CrearArticuloNew = () => {
  ...
  const handleAddSection = () => {
   ...
   const handleSelectSection = (e, index) => {
    ...
    const nextId = selectedIndex < listadoTitulos.length ? listadoTitulos[selectedIndex]._id : null;
      ...
      <div className='container CrearArticulo'>
        ...
        <div className='imgCont'>
          ...
          <div className={isInputEmpty ? "inputError" : ""}>
            <input
              type="text"
              placeholder= {isInputEmpty ? "Agrega un Titulo" : "Nombre del articulo"}
              className= {` inputZ ${isInputEmpty ? "inputError1 " : "imgText"}`}
              value={articleName}
              onChange={handleArticleName}
            />
            {isInputEmpty && <i className="bi bi-exclamation-circle circleIcon"></i>}
          </div>
        </div> 
          ...
          <motion.div className="slider_container">
            <motion.div className="slider" drag="x" dragConstraints={width}>
              {category.map((categoria) => {
                return (
                  <motion.div className={`itemCarousel ${selectedCategoryId === categoria._id ? 'active' : ''}`}>
                    <li
 ...
 */}          <div className="contenedor-NoCarousel">
        ...
        {secciones.map((seccion, index) => (
          <div className="selected " key={index}>
            <div className='selectedTitle'>
              ...
              >
                {listadoTitulos.map((listado) => (
                  <option key={listado._id} 
                          value={listado.sectionTitle}>
            ...
            <textarea
              className={`textArea shadow ${isTextEmpty ? "borderText" : "textArea"} `}
